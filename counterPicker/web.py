...
# Normalize the input hero names for comparison by replacing spaces with hyphens
normalized_hero_names = [name.replace(" ", "-").lower() for name in hero_names]
...
for hero, avg in sorted_advantages:
    # Normalize the hero name for comparison by replacing spaces with hyphens
    if hero.replace(" ", "-").lower() not in normalized_hero_names:  # Exclude input heroes
